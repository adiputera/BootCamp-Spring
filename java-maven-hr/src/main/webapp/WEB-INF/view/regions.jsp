...
$(document).ready(function(){
	...
	$('#data-regions').on('click', '.tbldelete',function(){
		var id = $(this).attr('id');
		$('#del-id').val(id);
	...
	$('#btn-hapus').on('click', function(){
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/regions/delete/'+id,
			type : 'DELETE',
	...
	$('#tbladd').on('click', function(){
		$('#judul-modal').html('Tambah Data Region');
		$('#in-id').val('');
		$('#in-name').val('');
		$('#form-regions').modal('show');
	});
	...
	$('#tblsimpan').on('click', function(){
		var regions = {
				id : $('#in-id').val(),
				regionName : $('#in-name').val(),
			}
			console.log(regions);
			validate = $('#add-regions').parsley();
			validate.validate();
			if(validate.isValid()){
				$.ajax({
					url : '${pageContext.request.contextPath}/regions/save',
					type : 'post',
	...
	$('#data-regions').on('click', '.tblupdate',function(){
		var id = $(this).attr('id');
		console.log('klik edit');
		$.ajax({
				url: '${pageContext.request.contextPath}/regions/get-one/'+id,
				type: "GET",
				dataType: "json",
				success:function(data) {	
					console.log('sukses ambil data');
					$('#in-id').val(data.id);
					$('#in-name').val(data.regionName);
					$("#form-regions").modal("show");
				}
		});
	});
...
<div class="container">
	...
	<table id="data-regions" class="table table-striped table-bordered"
		...
		<tbody>
			<c:forEach items="${regionss }" var="regions">
				<tr>
					<td>${regions.id }</td>
					<td>${regions.regionName }</td>
					<td><a href="#" id="${regions.id }"
						class="tbldelete btn btn-warning">Delete</a> | <a href="#"
						id="${regions.id }" class="tblupdate btn btn-success">Update</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>

	<div id="form-regions" class="modal" tabindex="-1" role="dialog">
				...
				<div class="modal-body">
					<form id="add-regions">
						...
						<div class="form-group">
							<label for="exampleInputEmail1">Region Name</label> <input type="text"
								class="form-control" id="in-name" placeholder="Region Name" data-parsley-required="true" data-parsley-length="[4,30]">
						</div>
					</form>
				</div>
